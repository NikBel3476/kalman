...
QByteArray Crc::crc(uint32_t padlen, const QByteArray &data) {
	auto state = _crc32(data, 0);
	for (size_t _i = data.length(); _i < padlen - 1; _i += 4) {
		const auto crcpad = QByteArray::fromStdString("\xff\xff\xff\xff");
		state = _crc32(crcpad, state);
	}
...
uint32_t Crc::_crc32(const QByteArray &bytes, uint32_t initial_state) {
